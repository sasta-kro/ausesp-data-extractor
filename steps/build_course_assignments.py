...
One prompt per group under .tmp-enrichment/prompts/course-group-NN.md. Each
assigned project has a rendered cover at .tmp-enrichment/render/<id>/report-p1.png.
Agents read the cover, capture the course statement verbatim, and write one
JSON per project to output/sp-course/<id>.json. Word-doc projects are excluded
...
1. Read .tmp-enrichment/render/{pid}/report-p1.png (the report cover). If the cover looks like it continues or the course line might sit on the approval page instead, also read report-p2.png and report-p3.png (they exist for every project).
...
Write to output/sp-course/{pid}.json:
{{
  "id": "{pid}",
...
def main() -> int:
    ...
    render = json.loads((base / "render-manifest.json").read_text())
    ...
    out_dir = root / "output" / "sp-course"
    out_dir.mkdir(exist_ok=True)

    groups = [ids[i:i + args.group_size] for i in range(0, len(ids), args.group_size)]
    for index, group in enumerate(groups, start=1):
        blocks = "\n".join(f"- {pid} -> write output/sp-course/{pid}.json" for pid in group)
